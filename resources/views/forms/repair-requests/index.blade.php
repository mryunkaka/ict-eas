            ...
            <div class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                <form method="GET" class="grid gap-4 md:grid-cols-[minmax(0,1fr)_180px_auto]">
                    ...
                    <div class="flex items-end gap-2">
                        ...
                        <x-button :href="route('forms.repairs.index')" variant="secondary"><x-heroicon-o-arrow-path class="mr-2 h-4 w-4" />Reset</x-button>
                    </div>
                </form>
                <x-button :href="route('forms.repairs.create')"><x-heroicon-o-plus class="mr-2 h-4 w-4" />Buat Permohonan</x-button>
            </div>
        </x-card>
        <x-table>
            <thead class="bg-ink-50 text-left text-ink-500">
                <tr>
                    <th class="px-4 py-3"><x-sort-link column="problem_summary" label="Masalah" :sort="$sort" :direction="$direction" /></th>
                    <th class="px-4 py-3">Asset</th>
                    <th class="px-4 py-3"><x-sort-link column="priority" label="Prioritas" :sort="$sort" :direction="$direction" /></th>
